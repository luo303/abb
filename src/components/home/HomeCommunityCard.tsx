import React from 'react'
import { View, Text, StyleSheet, Image, TouchableOpacity } from 'react-native'
import { Ionicons, FontAwesome } from '@expo/vector-icons'
import { useNavigation } from '@react-navigation/native'
import { NavigationProps } from '../../types/navigation'

export default function HomeCommunityCard() {
  const navigation = useNavigation<NavigationProps>()
  return (
    <TouchableOpacity
      style={styles.communityContainer}
      activeOpacity={0.9}
      onPress={() => navigation.navigate('PostDetail', { id: 1 })}
    >
      <View style={styles.cardContainer}>
        <View style={styles.header}>
          <Text style={styles.title}>社区模块</Text>
        </View>

        {/* 用户信息行 */}
        <View style={styles.userInfoRow}>
          <Image
            source={require('../../assets/icon.png')} // 临时头像
            style={styles.avatar}
          />
          <View style={styles.userInfo}>
            <Text style={styles.userName}>用户</Text>
            {/* <Text style={styles.postTime}>今天带宝宝来拍照啦！</Text> */}
          </View>
          <View style={styles.dateContainer}>
            <Text style={styles.dateText}>2026.01.19</Text>
            <Text style={styles.timeText}>23:09</Text>
          </View>
        </View>

        {/* 帖子内容 */}
        <Text style={styles.postContent}>今天带宝宝来拍照啦！</Text>

        {/* 图片网格 */}
        <View style={styles.imageGrid}>
          <Image
            source={{ uri: 'https://via.placeholder.com/100' }}
            style={styles.postImage}
          />
          <Image
            source={{ uri: 'https://via.placeholder.com/100' }}
            style={styles.postImage}
          />
          <Image
            source={{ uri: 'https://via.placeholder.com/100' }}
            style={styles.postImage}
          />
        </View>

        {/* 底部交互栏 */}
        <View style={styles.actionRow}>
          <View style={styles.actionItem}>
            <Ionicons name="heart-outline" size={20} color="#666" />
            <Text style={styles.actionText}>11</Text>
          </View>
          <View style={styles.actionItem}>
            <Ionicons name="heart-dislike-outline" size={20} color="#666" />
            <Text style={styles.actionText}>1</Text>
          </View>
          <View style={styles.actionItem}>
            <Ionicons name="star-outline" size={20} color="#666" />
            <Text style={styles.actionText}>6</Text>
          </View>
          <View style={styles.actionItem}>
            <Ionicons name="chatbubble-outline" size={20} color="#666" />
            <Text style={styles.actionText}>2</Text>
          </View>
        </View>
      </View>
    </TouchableOpacity>
  )
}

const styles = StyleSheet.create({
  communityContainer: {
    // 确保点击区域正常
  },
  cardContainer: {
    backgroundColor: '#fff',
    borderRadius: 15,
    marginHorizontal: 15,
    padding: 15,
    marginBottom: 20,
    // Shadow
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.1,
    shadowRadius: 3,
    elevation: 3
  },
  header: {
    marginBottom: 15
  },
  title: {
    fontSize: 16,
    fontWeight: 'bold',
    color: '#333'
  },
  userInfoRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 10
  },
  avatar: {
    width: 40,
    height: 40,
    borderRadius: 20,
    backgroundColor: '#eee'
  },
  userInfo: {
    marginLeft: 10,
    flex: 1
  },
  userName: {
    fontSize: 14,
    color: '#555',
    fontWeight: '500'
  },
  dateContainer: {
    alignItems: 'flex-end'
  },
  dateText: {
    fontSize: 12,
    color: '#999'
  },
  timeText: {
    fontSize: 12,
    color: '#999'
  },
  postContent: {
    fontSize: 15,
    color: '#333',
    marginBottom: 10,
    fontWeight: 'bold'
  },
  imageGrid: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginBottom: 15
  },
  postImage: {
    width: '31%', // 约三分之一
    aspectRatio: 1,
    borderRadius: 8,
    backgroundColor: '#f0f0f0'
  },
  actionRow: {
    flexDirection: 'row',
    justifyContent: 'flex-end',
    gap: 15
  },
  actionItem: {
    flexDirection: 'row',
    alignItems: 'center'
  },
  actionText: {
    fontSize: 12,
    color: '#666',
    marginLeft: 4
  }
})
